...
with tab1:
    id_busca = st.text_input("🔎 Bipar Pedido ou Gaiola:", key="input_aloc_vfinal").strip()
    if id_busca:
        # BUSCA INTEGRADA NAS DUAS BASES
        ...
        if not aloc_alvo.empty:
            ...
            for i, row in enumerate(sugestoes.itertuples()):
                with cols[i]:
                    # Exibição do corridor_cage em destaque absoluto
                    st.markdown(f"""
                    <div style="background:#1e1e1e; padding:20px; border-radius:15px; border: 3px solid #ff4b4b; text-align:center; min-height:220px">
                        <p style="color:gray; font-size:12px; margin:0">CORREDOR / GAIOLA</p>
                        <h1 style="color:#ff4b4b; margin:5px 0; font-size:60px; font-weight:black">{row.corridor_cage}</h1>
                        <hr style="border:0.5px solid #333; margin:10px 0">
                        <p style="color:white; font-size:18px; margin:0">Distância: <b>{row.dist_km:.2f} km</b></p>
                    </div>
                    """, unsafe_allow_html=True)
                    
                    # Ações de Alocação e Impressão
                    id_shopee = getattr(row, 'planned_at', '')
                    st.link_button("📥 Acessar Task Shopee", f"https://spx.shopee.com.br/#/assignment-task/detailNoLabel?id={id_shopee}", use_container_width=True)
                    if st.button(f"🖨️ Imprimir Etiqueta {row.corridor_cage}", key=f"prt_final_{i}", use_container_width=True):
                        st.balloons()
                        st.toast(f"Etiqueta {row.corridor_cage} enviada!", icon="✅")
        else:
            st.error("❌ Código não encontrado nas bases de dados.")
with tab2:
